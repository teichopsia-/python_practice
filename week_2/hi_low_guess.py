user_guess = int(raw_input('Please pick a number between 0 and 100'))

low = 0
high = 100
ans = (high + low) / 2

while ans != user_guess:
    print 'is your secret number: ' + str(ans) + '?'
    ask = str(raw_input("'h' too high, 'l' too low, 'c' correct"))
    if ask == 'h':
        high = ans
    elif ask == 'l':
        low = ans
    else:
        print 'please pick one of "h", "l" or "c"'
        continue
    ans = (high + low) / 2
